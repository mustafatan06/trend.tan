      ...
      <div className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input 
              type="text" 
              value={`${form.ad} ${form.soyad}`} 
              disabled 
              className="w-full p-3 bg-gray-100 rounded-lg border text-gray-700 font-medium" 
            />
          </div>
          <div>
          ...
          <div>
            ...
            <input 
              type="text" 
              required 
              onChange={(e) => setForm({...form, magazaAdi: e.target.value})} 
          ...
          <button 
            type="submit" 
            className="w-full bg-orange-600 text-white p-3 rounded-lg font-bold hover:bg-orange-700 transition cursor-pointer"
          >
            Başvuruyu Tamamla
          </button>
        </form>
        <Link href="/" className="block mt-4 text-center text-sm text-gray-500 hover:text-black">
          ← Ana Sayfaya Dön
        </Link>
      </div>
    </div>
