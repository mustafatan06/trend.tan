...
export default function AdminPage() {
  // Giriş yapılmış mı kontrolü için state
  const [isLoggedIn, setIsLoggedIn] = useState(false);
  
  // Giriş form bilgileri
  const [loginForm, setLoginForm] = useState({
    kullaniciAdi: "",
    sifre: "",
  });

  // Başvuru form bilgileri (Bilgilerin otomatik dolu geliyor)
  const [basvuruForm, setBasvuruForm] = useState({
    adSoyad: "Mustafa Tan",
    tcVergiNo: "66139240304",
    magazaAdi: "",
    dogumTarihi: "02/07/1997",
    iban: "TR08 0015 7000 0000 0143 0070 74",
    adres: "Beşikkaya Mahallesi 1947. Sokak 33/5 Altındağ / Ankara",
    telefon: "05XXXXXXXXX",
  });

  // Ürün ekleme paneli için state'ler
  const [urunler, setUrunler] = useState<string[]>([]);
  const [yeniUrun, setYeniUrun] = useState("");

  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    // Belirttiğin kullanıcı adı ve şifre kontrolü
    if (loginForm.kullaniciAdi === "admin" && loginForm.sifre === "123456") {
      setIsLoggedIn(true);
      alert("Yönetici paneline başarıyla giriş yaptınız!");
    } else {
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-10 px-4 flex flex-col items-center">
      ...
      {/* DURUM 1: GİRİŞ YAPILDIYSA ÜRÜN EKLEME / YÖNETİM PANELİ GÖRÜNÜR */}
      {isLoggedIn ? (
        ...
        /* DURUM 2: GİRİŞ YAPILMADIYSA DETAYLI BAŞVURU FORMU GÖRÜNÜR */
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full p-3 bg-gray-50 rounded-xl border text-sm font-medium"
                  required
                />
              </div>
              <div>
